...
bool init()
{
	...
	{
		//Create window
		gWindow = SDL_CreateWindow( "Game Over", SCREEN_X, SCREEN_Y, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_BORDERLESS );
		if( gWindow == NULL )
		{
...
bool loadMedia()
{
	...
	//Load splash image
	image = SDL_LoadBMP( "pictures/gameOverImage.bmp" );
	if( image == NULL )
	{
		printf( "Unable to load image %s! SDL Error: %s\n", "pictures/gameOver.bmp", SDL_GetError() );
		success = false;
	}
