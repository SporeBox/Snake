...
bool init()
{
	...
	{
		//Create window
		gWindow = SDL_CreateWindow( "Player 1 Wins", SCREEN_X, SCREEN_Y, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_BORDERLESS );
		if( gWindow == NULL )
		{
...
bool loadMedia()
{
	...
	//Load splash image
	image = SDL_LoadBMP( "pictures/p1w.bmp" );
	if( image == NULL )
	{
		printf( "Unable to load image %s! SDL Error: %s\n", "pictures/p1w.bmp", SDL_GetError() );
		success = false;
	}
